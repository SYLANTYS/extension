...
function App() {
  const [apiData, setApiData] = useState<string>(
    "Using OpenAI's 3.5 Turbo Instruct Model to generate company summaries. Enjoy!"
  );
  ...
  const fetchOpenAiData = async (prompt: string): Promise<string> => {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        prompt:
          "create a company summary for this ticker in two sentences (write the name of company first): " +
          prompt,
        max_tokens: 100,
        temperature: 0,
      }),
  ...
  const fetchOptionsData = async (expirationDate: number) => {
    const response = await fetch(
      `https://yfapi.net/v7/finance/options/${customPrompt}?date=${expirationDate}`,
      {
        headers: {
  ...
  const fetchExpirationDates = async (ticker: string) => {
    ...
    const response = await fetch(
      `https://yfapi.net/v7/finance/options/${ticker}`,
      {
        headers: {
          accept: "application/json",
          "X-API-KEY": process.env.REACT_APP_YAHOO_FINANCE_API_KEY,
  ...
  const handleTypeOfOptionsChange = (
    event: React.ChangeEvent<HTMLSelectElement>
  ) => {
  ...
  const getFilteredOptions = (): Option[] => {
    ...
    const numOptions =
      numberOfOptions === "all" ? data.length : parseInt(numberOfOptions, 10);
    ...
    if (typeOfOptions === "calls") {
      return [
        ...inTheMoneyOptions.slice(
          inTheMoneyOptions.length -
            (numberOfOptions === "all" || inTheMoneyOptions.length < numOptions
              ? inTheMoneyOptions.length
              : numOptions),
          inTheMoneyOptions.length
        ),
      ...
      return [
        ...outOfTheMoneyOptions.slice(
          outOfTheMoneyOptions.length -
            (numberOfOptions === "all" ||
            outOfTheMoneyOptions.length < numOptions
              ? outOfTheMoneyOptions.length
              : numOptions),
          outOfTheMoneyOptions.length
        ),
    ...
    <div className="container p-4 pt-4 mx-auto bg-gray-900 min-w-[320px]">
      {askPrice !== null && (
        <div className="flex items-center mb-4 text-2xl font-light">
          <p
            className={`text-left ${priceColor} transition-colors duration-500`}
          >
      ...
      <div className="mb-4 flex justify-center items-center">
        <div className="flex justify-center border border-gray-300 rounded-xl bg-gray-700">
          <input
            ...
            value={customPrompt}
            onChange={(e) =>
              setCustomPrompt(e.currentTarget.value.toUpperCase())
            }
      ...
      {selectedDate && (selectedDate + 100800) * 1000 < new Date().getTime() ? (
        <p className="text-red-500">EXPIRED CONTRACTS RESET HERE ⬆️⬆️⬆️</p>
      ) : (
        ""
      )}
      ...
      {callsData.length > 0 ? (
        <div>
          ...
          <OptionDetails
            ...
            onContractSymbolClick={handleContractSymbolClick}
          />
        </div>
      ) : (
        <div>
