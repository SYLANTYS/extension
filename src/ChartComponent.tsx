      ...
      <div className='flex justify-between'>
        <div className="flex justify-center mb-2">
          {ranges.map((r) => (
            <button
              key={r.label}
              onClick={() => setSelectedRange({ range: r.range, interval: r.interval })}
              className={`px-2 py-1 m-1 text-xs border rounded ${selectedRange.range === r.range ? 'bg-blue-500 text-white' : 'bg-gray-700 text-white'} hover:bg-gray-600`}
            >
              {r.label}
            </button>
          ))}
                ...
                tooltip: {
                  callbacks: {
                    ...
                    label: function(tooltipItem: any) {
                      ...
                      const point = chartData.info[dataIndex];
                      return [
                        `Close: ${point.close !== null ? point.close.toFixed(2) : 'N/A'}`,
                        `Open: ${point.open !== null ? point.open.toFixed(2) : 'N/A'}`,
                        `High: ${point.high !== null ? point.high.toFixed(2) : 'N/A'}`,
                        `Low: ${point.low !== null ? point.low.toFixed(2) : 'N/A'}`,
                        `Volume: ${point.volume !== null ? point.volume : 'N/A'}`
                      ];
                    }
                  }
                }
